...
struct ContentView: View {
    ...
    */
        ...
        for note in sheetNotesToPlay {
            ...
            let spawnWork = DispatchWorkItem {
                DispatchQueue.main.async {
                    ...
                    if !isHoldNote { DispatchQueue.main.asyncAfter(deadline: .now() + self.lifeDuration, execute: deleteWork) }
                }
            } // end spawnWork
    ...
    private func handleHoldTouchEnded(at location: CGPoint) {
        ...
        guard let holdEnd = holdEndRef else { showJudgement(text: "OK", color: .white); return }
    ...
    private func handleTap(at location: CGPoint, in _unused: CGPoint) {
        ...
        for (i, a) in activeNotes.enumerated() {
            ...
            let topRect = CGRect(x: finalTop.x - 44/2.0, y: finalTop.y - triangleH/2.0, width: 44, height: triangleH)
            let bottomRect = CGRect(x: finalBottom.x - 44/2.0, y: finalBottom.y - triangleH/2.0, width: 44, height: triangleH)
